...
\begin{LeanVerbatim}
\textbf{def} or := (\textit{·} \symbol{124}\symbol{124} \textit{·})
...
\LeanVerb|ih| records that reversing the tail twice gives back the original tail.\begin{LeanVerbatim}
       
      \textbf{simp} [List.reverse_cons, \textit{ih}]
   
\end{LeanVerbatim}
The final \LeanVerb|simp| unfolds \LeanVerb|List.reverse_cons| and uses the induction hypothesis to collapse the
